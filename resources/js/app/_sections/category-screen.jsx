...
export function CategoryScreen({ categoryFilter, searchText, setSearchText, onGo, onOpenVenue, selected }) {
    ...
    console.log('venues', selected)
    const venues = selected?.venues
    return (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    <div
                        ...
                        style={{ background: CATEGORY_COLORS[selected.key] || "var(--amber)" }}
                    >
                        <ServiceCategoryIcon category={selected.key} className="w-6 h-6 stroke-white" />
                    </div>
                    <div>
                        <h1 className="mp-display text-3xl text-[var(--pitch)]">{selected.name} Spaces</h1>
                        ...
                        <div key={v.id} onClick={() => onOpenVenue(v.id)} className="venue-card-hover bg-white rounded-3xl border border-[var(--line-dark-strong)] p-6 cursor-pointer flex flex-col justify-between">
                            ...
                            <div className="pt-4 border-t border-[var(--line-dark)] flex items-center justify-between">
                                <div>
                                    ...
                                    <span className="mp-display text-xl text-[var(--pitch)]">${v.base_price}<span className="text-xs font-normal">/{v.unitLabel ?? 'Hour'}</span></span>
                                </div>
